...
- `prototype_editor/modules/preview_regression.js` owns automatic preview-regression seed selection, branch-choice targeting, and best-effort condition-variable presets for route smoke tests. Keep future route-smoke prioritization there so `app.js` only runs the selected seeds through the editor preview runtime.
