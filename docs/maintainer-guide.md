...
- `prototype_editor/modules/playtest_handoff_report.js` owns tester-facing playtest handoff exports: route cases, regression smoke results, fix queues, Markdown work orders, feedback templates, and CSV rows. Keep tester handoff wording and table formats there so `app.js` only wires buttons and downloads.
