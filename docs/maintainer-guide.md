...
- `prototype_editor/modules/preview_regression.js` owns automatic preview-regression seed selection and branch-choice targeting. Keep future route-smoke prioritization there so `app.js` only runs the selected seeds through the editor preview runtime.
