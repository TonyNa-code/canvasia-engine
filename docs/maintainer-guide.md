...
- `prototype_editor/modules/scene_production_board.js` owns scene-level production task analysis and export formatting: per-scene completion scoring, next-action suggestions, recommended scene recipes, missing background / BGM / voice checks, text-length checks, choice crowding checks, bad route targets, Markdown, and CSV. Keep production-first diagnostics there, but reuse `story_templates.js` for scene-aware recipe scoring so the story page, command palette, and production board do not drift into separate recommendation systems.
...
- `prototype_editor/modules/runtime_preload_budget.js` owns editor-side Runtime startup-pressure analysis and export formatting: entry-scene / early-route preload candidates, first-screen byte budgets, missing first-screen files, single-asset preload risks, scene hotspots, Markdown, and CSV. Keep future startup-loading advice here so `app.js` remains a thin inspection card and export button shell, and keep the phase rules aligned with `export_runtime_preload.py`.
...
- `prototype_editor/modules/runtime_capability_matrix.js` owns editor-to-runtime coverage reporting and export acceptance guidance: project block usage, Web Runtime coverage, native Runtime coverage, partial / unknown support detection, generated Runtime acceptance checklist items, Markdown, and CSV. Keep block support declarations sourced from `story_block_catalog.js`, and put only matrix-specific QA wording or acceptance checklist logic here.
...
- `prototype_editor/modules/production_backlog.js` owns cross-module production task aggregation and export formatting: route, scene, director cue, voice, choice, variable, asset dependency, asset rights, audio, stage, presentation, localization, runtime coverage, Runtime preload budget, and unlockable-content issues are normalized into one prioritized work queue with Markdown and CSV exports. Keep future task scoring / ownership rules there instead of duplicating queue logic across inspection cards.
