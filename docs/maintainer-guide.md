...
- `prototype_editor/modules/asset_rights_sheet.js` owns release-facing asset rights and credits analysis plus the asset-detail rights form: license labels, source / author records, commercial-use status, attribution needs, placeholder markers, AI provenance hints, Staff / Credits rows, Markdown, CSV, form-field collection, and the inspection-page card. Keep future publishing-rights QA there; `asset_dependency_sheet.js` should remain focused on whether assets are referenced and safe to delete.
