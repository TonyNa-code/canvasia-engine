...
- `prototype_editor/modules/scene_production_board.js` owns scene-level production task analysis and export formatting: per-scene completion scoring, next-action suggestions, missing background / BGM / voice checks, text-length checks, choice crowding checks, bad route targets, Markdown, and CSV. Keep future scene workboard logic there so route analysis, story editing, and inspection UI stay focused.
...
- `prototype_editor/modules/runtime_capability_matrix.js` owns editor-to-runtime coverage reporting: project block usage, Web Runtime coverage, native Runtime coverage, partial / unknown support detection, Markdown, and CSV. Keep future card-type support declarations there whenever new story block types are added, so export confidence does not depend on scattered manual checks.
