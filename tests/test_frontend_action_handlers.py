...
class FrontendActionHandlerTests(unittest.TestCase):
    ...
    def test_export_player_surfaces_runtime_preload_size_context(self) -> None:
        ...
        remember_assets = _extract_function_source(source, "rememberRuntimePreloadAssets")
        cached_assets = _extract_function_source(source, "getRuntimeCachedAssetIds")
        meta_text = _extract_function_source(source, "getRuntimePreloadMetaText")

        self.assertIn("buildRuntimePreloadStatusText", source)
        self.assertIn("buildRuntimePreloadMetaText", source)
        self.assertIn("buildRuntimePreloadStatusText({", status_text)
        self.assertIn("manifest: data.buildInfo.runtimePreloadManifest", status_text)
        self.assertIn("preloadStatus: state.runtimePreloadStatus", status_text)
        self.assertIn("prefetchStatus: state.runtimeScenePrefetchStatus", status_text)
        self.assertIn("rememberRuntimePreloadAssets(status, state.runtimePreloadedAssetIds)", start_preload)
        self.assertIn("const cachedAssetIds = getRuntimeCachedAssetIds()", start_prefetch)
        self.assertIn("excludeAssetIds: cachedAssetIds", start_prefetch)
        self.assertIn("skipAssetIds: cachedAssetIds", start_prefetch)
        self.assertIn("loadedAssetIds", remember_assets)
        self.assertIn("skippedAssetIds", remember_assets)
        self.assertIn("state.runtimePreloadedAssetIds", cached_assets)
        self.assertIn("state.runtimeScenePrefetchedAssetIds", cached_assets)
        self.assertIn("buildRuntimePreloadMetaText(data.buildInfo.runtimePreloadManifest)", meta_text)
