...
class FrontendActionHandlerTests(unittest.TestCase):
    ...
    def test_command_palette_wiring_stays_integrated_with_editor_actions(self) -> None:
        ...
        self.assertIn("finalizeStoryTemplateBlocks(blocks, recipes)", build_story_template_blocks)
        ...
        self.assertIn('if (!String(block.position ?? "").trim())', apply_story_template_speaker)
        self.assertIn("block.position = getDefaultCharacterPosition(speakerId)", apply_story_template_speaker)
        self.assertIn("choiceOptions", apply_story_template_choice_options)
        self.assertIn("buildStoryTemplateChoiceEffect", apply_story_template_choice_options)
        self.assertIn('block.endMode = "after_block"', finalize_story_template_blocks)
        self.assertIn("block.endBlockId = endBlock.id", finalize_story_template_blocks)
