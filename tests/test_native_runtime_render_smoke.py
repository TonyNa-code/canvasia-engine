...
class NativeRuntimeRenderSmokeTests(unittest.TestCase):
    ...
    def test_native_runtime_renders_ui_skin_overlays_headlessly(self) -> None:
        ...
        self.assertEqual(player.get_setting_value_label("readingProfile"), "自定义组合")
        player.adjust_runtime_setting("readingProfile", 1)
        self.assertEqual(player.get_setting_value_label("readingProfile"), "原作演出")
        self.assertEqual(player.runtime_settings["textScalePercent"], 100)
        self.assertEqual(player.runtime_settings["visualComfort"], "standard")
        player.adjust_runtime_setting("readingProfile", 1)
        self.assertEqual(player.get_setting_value_label("readingProfile"), "舒适阅读")
        self.assertEqual(player.runtime_settings["textScalePercent"], 110)
        self.assertEqual(player.runtime_settings["visualComfort"], "gentle")
