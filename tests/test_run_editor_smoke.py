...
class RunEditorSmokeTests(unittest.TestCase):
    ...
    def assert_unlockable_content_manifest_file(self, manifest_path: Path) -> dict:
        ...
        self.assertIn("chapter_replay", group_ids)
        self.assertIn("achievements", group_ids)
    ...
    def assert_export_playtest_guide_file(self, guide_path: Path) -> str:
        self.assertTrue(guide_path.is_file())
        content = guide_path.read_text(encoding="utf-8")
        self.assertIn("# 试玩与发布验收指南", content)
        self.assertIn("## 打开方式", content)
        self.assertIn("## 先验这几项", content)
    ...
    def assert_export_release_evidence_pack_file(self, evidence_path: Path) -> str:
        ...
        self.assertIn(run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME, content)
    ...
    def assert_export_stage_direction_files(self, build_dir: Path) -> dict:
        manifest_path = build_dir / run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME
        report_path = build_dir / run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME
        csv_path = build_dir / run_editor.EXPORT_STAGE_DIRECTION_CSV_NAME
        self.assertTrue(manifest_path.is_file())
        self.assertTrue(report_path.is_file())
        self.assertTrue(csv_path.is_file())
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        ...
        self.assertEqual(payload["formatVersion"], 1)
        self.assertIn("releaseReadinessPercent", payload["summary"])
        self.assertIn("角色舞台调度表", report)
        self.assertIn("status,type,chapter", csv_text)
        return payload

    def assert_export_presentation_timeline_files(self, build_dir: Path) -> dict:
        manifest_path = build_dir / run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME
        report_path = build_dir / run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME
        csv_path = build_dir / run_editor.EXPORT_PRESENTATION_TIMELINE_CSV_NAME
        self.assertTrue(manifest_path.is_file())
        self.assertTrue(report_path.is_file())
        self.assertTrue(csv_path.is_file())
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        ...
        self.assertEqual(payload["formatVersion"], 1)
        self.assertIn("releaseReadinessPercent", payload["summary"])
        self.assertIn("演出时间轴", report)
        self.assertIn("序号,章节,场景", csv_text)
        return payload

    def assert_export_route_playtest_workbook_files(self, build_dir: Path) -> dict:
        manifest_path = build_dir / run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME
        report_path = build_dir / run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME
        csv_path = build_dir / run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_CSV_NAME
        self.assertTrue(manifest_path.is_file())
        self.assertTrue(report_path.is_file())
        self.assertTrue(csv_path.is_file())
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        ...
        self.assertEqual(payload["formatVersion"], 1)
        self.assertIn("readinessPercent", payload["summary"])
        self.assertIn("路线试玩工作簿", report)
        self.assertIn("类型,序号,章节", csv_text)
        return payload

    def assert_export_choice_consequence_files(self, build_dir: Path) -> dict:
        manifest_path = build_dir / run_editor.EXPORT_CHOICE_CONSEQUENCE_JSON_NAME
        report_path = build_dir / run_editor.EXPORT_CHOICE_CONSEQUENCE_REPORT_NAME
        csv_path = build_dir / run_editor.EXPORT_CHOICE_CONSEQUENCE_CSV_NAME
        self.assertTrue(manifest_path.is_file())
        self.assertTrue(report_path.is_file())
        self.assertTrue(csv_path.is_file())
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    ...
    def assert_export_release_readiness_files(self, summary_path: Path, report_path: Path) -> dict:
        ...
        report = report_path.read_text(encoding="utf-8")
        self.assertEqual(payload["formatVersion"], 1)
        self.assertIn(payload["qualityGate"]["status"], {"ready", "needs_review", "blocked"})
    ...
    def assert_export_story_route_map_files(self, map_path: Path, report_path: Path) -> dict:
        ...
        report = report_path.read_text(encoding="utf-8")
        self.assertEqual(payload["formatVersion"], 1)
        self.assertIn(payload["summary"]["status"], {"ready", "needs_review", "blocked"})
        self.assertIn("sceneCount", payload["summary"])
        self.assertIn("# 剧情路线图随包报告", report)
    ...
    def test_export_renpy_draft_build_creates_starter_bundle(self) -> None:
        ...
        screens_path = build_dir / "game" / run_editor.RENPY_SCREENS_FILE_NAME
        ...
        self.assertTrue(screens_path.is_file())
        ...
        self.assertEqual(export_result["renpyQualityStatus"], "ready")
        self.assertEqual(export_result["renpyQualitySummary"]["errorCount"], 0)
        self.assertEqual(export_result["renpyQualitySummary"]["reviewCount"], 0)
        ...
        self.assertIn("fadein 0.8 noloop if_changed volume 0.82", script)
        ...
        self.assertIn('play canvasia_effect_1 "assets/sfx/', script)
        ...
        self.assertEqual(manifest["files"]["renpyScreens"], f"game/{run_editor.RENPY_SCREENS_FILE_NAME}")
        ...
        self.assertIn(f"game/{run_editor.RENPY_SCREENS_FILE_NAME}", names)
    ...
    def test_web_export_build_smoke(self) -> None:
        ...
        self.assertTrue((build_dir / "runtime_visual_comfort.js").is_file())
        ...
        self.assertTrue((build_dir / "runtime_rich_text.js").is_file())
        self.assertTrue((build_dir / "runtime_story_text.js").is_file())
        self.assertTrue((build_dir / "runtime_text_history.js").is_file())
        self.assertTrue((build_dir / "runtime_save_slots.js").is_file())
        ...
        stage_direction_payload = self.assert_export_stage_direction_files(build_dir)
        self.assertEqual(export_result["stageDirectionName"], run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME)
        self.assertEqual(export_result["stageDirectionReportName"], run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME)
        presentation_timeline_payload = self.assert_export_presentation_timeline_files(build_dir)
        self.assertEqual(export_result["presentationTimelineName"], run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME)
        self.assertEqual(export_result["presentationTimelineReportName"], run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME)
        route_playtest_payload = self.assert_export_route_playtest_workbook_files(build_dir)
        self.assertEqual(export_result["routePlaytestWorkbookName"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        self.assertEqual(export_result["routePlaytestWorkbookReportName"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME)
        self.assertEqual(route_playtest_payload["summary"]["routeCaseCount"], 0)
        choice_consequence_payload = self.assert_export_choice_consequence_files(build_dir)
        ...
        self.assertEqual(
            ...
            index_payload["buildInfo"]["unlockableContentManifest"]["summary"]["totalEntryCount"],
        )
        ...
        self.assertEqual(manifest["files"]["playerRuntimeReadingProfiles"], "runtime_reading_profiles.js")
        self.assertEqual(manifest["files"]["playerRuntimeDialogueLayouts"], "runtime_dialogue_layouts.js")
        self.assertEqual(manifest["files"]["playerRuntimeVisualComfort"], "runtime_visual_comfort.js")
        ...
        self.assertEqual(manifest["files"]["playerRuntimeRichText"], "runtime_rich_text.js")
        self.assertEqual(manifest["files"]["playerRuntimeStoryText"], "runtime_story_text.js")
        self.assertEqual(manifest["files"]["playerRuntimeTextHistory"], "runtime_text_history.js")
        self.assertEqual(manifest["files"]["playerRuntimeSaveSlots"], "runtime_save_slots.js")
        ...
        self.assertEqual(manifest["files"]["stageDirection"], run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME)
        self.assertEqual(manifest["files"]["stageDirectionReport"], run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME)
        self.assertEqual(manifest["files"]["stageDirectionCsv"], run_editor.EXPORT_STAGE_DIRECTION_CSV_NAME)
        self.assertEqual(manifest["files"]["presentationTimeline"], run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME)
        self.assertEqual(manifest["files"]["presentationTimelineReport"], run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME)
        self.assertEqual(manifest["files"]["presentationTimelineCsv"], run_editor.EXPORT_PRESENTATION_TIMELINE_CSV_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbook"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbookReport"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbookCsv"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_CSV_NAME)
        ...
        provenance = self.assert_export_provenance_file(
            export_result,
            {
                ...
                run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME,
                run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME,
                run_editor.EXPORT_STAGE_DIRECTION_CSV_NAME,
                run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME,
                run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME,
                run_editor.EXPORT_PRESENTATION_TIMELINE_CSV_NAME,
        ...
        self.assertGreaterEqual(stage_direction_payload["summary"]["eventCount"], 0)
        self.assertGreaterEqual(presentation_timeline_payload["summary"]["eventCount"], 0)
    ...
    def test_native_runtime_export_build_smoke(self) -> None:
        ...
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_SYSTEM_MENU_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_SAVE_VAULT_NAME).is_file())
        ...
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_TEXT_HISTORY_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_TEXT_HISTORY_OVERLAY_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_SURFACE_CACHE_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_DIALOG_PANEL_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_SAVE_SLOTS_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_SAVE_OVERLAY_NAME).is_file())
        ...
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_RICH_TEXT_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_STORY_TEXT_NAME).is_file())
        self.assertTrue((build_dir / run_editor.NATIVE_RUNTIME_RICH_TEXT_RENDERER_NAME).is_file())
        ...
        self.assertEqual(export_result["stageDirectionName"], run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME)
        self.assertEqual(export_result["stageDirectionReportName"], run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME)
        self.assertGreaterEqual(stage_direction_payload["summary"]["eventCount"], 1)
        presentation_timeline_payload = self.assert_export_presentation_timeline_files(build_dir)
        self.assertEqual(export_result["presentationTimelineName"], run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME)
        self.assertEqual(export_result["presentationTimelineReportName"], run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME)
        self.assertGreaterEqual(presentation_timeline_payload["summary"]["eventCount"], 1)
        route_playtest_payload = self.assert_export_route_playtest_workbook_files(build_dir)
        self.assertEqual(export_result["routePlaytestWorkbookName"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        self.assertEqual(export_result["routePlaytestWorkbookReportName"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME)
        self.assertGreaterEqual(route_playtest_payload["summary"]["routeCaseCount"], 0)
        choice_consequence_payload = self.assert_export_choice_consequence_files(build_dir)
        self.assertEqual(export_result["choiceConsequenceName"], run_editor.EXPORT_CHOICE_CONSEQUENCE_JSON_NAME)
        self.assertEqual(export_result["choiceConsequenceReportName"], run_editor.EXPORT_CHOICE_CONSEQUENCE_REPORT_NAME)
        ...
        self.assertEqual(
            runtime_preload_markdown.returncode,
            0,
            runtime_preload_markdown.stdout + runtime_preload_markdown.stderr,
        )
        self.assertIn("Runtime 资源预热报告", runtime_preload_markdown.stdout)
        self.assertIn("体积预算", runtime_preload_markdown.stdout)
        preload_report_text = (build_dir / run_editor.RUNTIME_PRELOAD_REPORT_FILE_NAME).read_text(encoding="utf-8")
        self.assertIn("Runtime 资源预热报告", preload_report_text)
        self.assertIn("体积预算", preload_report_text)
        ...
        self.assertTrue(
            any(item["name"] == export_result["playtestGuideName"] for item in release_artifact_payload["insideArchiveReports"])
        )
        self.assertTrue(
            any(
        ...
        self.assertTrue(
            any(item["name"] == export_result["stageDirectionReportName"] for item in release_artifact_payload["insideArchiveReports"])
        )
        self.assertTrue(
            any(item["name"] == export_result["presentationTimelineReportName"] for item in release_artifact_payload["insideArchiveReports"])
        )
        self.assertTrue(
            any(item["name"] == export_result["routePlaytestWorkbookReportName"] for item in release_artifact_payload["insideArchiveReports"])
        )
        self.assertTrue(
        ...
        self.assertEqual(manifest["files"]["stageDirectionReport"], run_editor.EXPORT_STAGE_DIRECTION_REPORT_NAME)
        self.assertEqual(manifest["files"]["stageDirectionCsv"], run_editor.EXPORT_STAGE_DIRECTION_CSV_NAME)
        self.assertEqual(manifest["runtime"]["stageDirection"], run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME)
        self.assertEqual(manifest["files"]["presentationTimeline"], run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME)
        self.assertEqual(manifest["files"]["presentationTimelineReport"], run_editor.EXPORT_PRESENTATION_TIMELINE_REPORT_NAME)
        self.assertEqual(manifest["files"]["presentationTimelineCsv"], run_editor.EXPORT_PRESENTATION_TIMELINE_CSV_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbook"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbookReport"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_REPORT_NAME)
        self.assertEqual(manifest["files"]["routePlaytestWorkbookCsv"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_CSV_NAME)
        self.assertEqual(manifest["runtime"]["routePlaytestWorkbook"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        self.assertEqual(manifest["files"]["choiceConsequence"], run_editor.EXPORT_CHOICE_CONSEQUENCE_JSON_NAME)
        self.assertEqual(manifest["files"]["choiceConsequenceReport"], run_editor.EXPORT_CHOICE_CONSEQUENCE_REPORT_NAME)
        self.assertEqual(manifest["files"]["choiceConsequenceCsv"], run_editor.EXPORT_CHOICE_CONSEQUENCE_CSV_NAME)
        self.assertEqual(manifest["files"]["variableInfluence"], run_editor.EXPORT_VARIABLE_INFLUENCE_JSON_NAME)
        ...
        self.assertEqual(manifest["files"]["runtimeRichTextModule"], run_editor.NATIVE_RUNTIME_RICH_TEXT_NAME)
        self.assertEqual(manifest["files"]["runtimeStoryTextModule"], run_editor.NATIVE_RUNTIME_STORY_TEXT_NAME)
        self.assertEqual(
            manifest["files"]["runtimeRichTextRendererModule"],
        ...
        self.assertEqual(manifest["files"]["runtimeSurfaceCacheModule"], run_editor.NATIVE_RUNTIME_SURFACE_CACHE_NAME)
        self.assertEqual(manifest["files"]["runtimeDialogPanelModule"], run_editor.NATIVE_RUNTIME_DIALOG_PANEL_NAME)
        ...
        self.assertEqual(manifest["runtime"]["runtimeSurfaceCacheModule"], run_editor.NATIVE_RUNTIME_SURFACE_CACHE_NAME)
        self.assertEqual(manifest["runtime"]["runtimeDialogPanelModule"], run_editor.NATIVE_RUNTIME_DIALOG_PANEL_NAME)
        ...
        self.assertTrue(export_result["runtimeViewModulePublicUrl"].endswith(run_editor.NATIVE_RUNTIME_VIEW_NAME))
        self.assertTrue(export_result["runtimeVnQualityModulePublicUrl"].endswith(run_editor.NATIVE_RUNTIME_VN_QUALITY_NAME))
    ...
    def test_windows_nwjs_build_smoke(self) -> None:
        ...
        self.assertTrue((build_dir / "app" / "runtime_rich_text.js").is_file())
        self.assertTrue((build_dir / "app" / "runtime_story_text.js").is_file())
        ...
        self.assertEqual(export_result["audioCueSheetName"], run_editor.EXPORT_AUDIO_CUE_SHEET_JSON_NAME)
        self.assert_export_stage_direction_files(build_dir)
        self.assertEqual(export_result["stageDirectionName"], run_editor.EXPORT_STAGE_DIRECTION_JSON_NAME)
        self.assert_export_presentation_timeline_files(build_dir)
        self.assertEqual(export_result["presentationTimelineName"], run_editor.EXPORT_PRESENTATION_TIMELINE_JSON_NAME)
        self.assert_export_route_playtest_workbook_files(build_dir)
        self.assertEqual(export_result["routePlaytestWorkbookName"], run_editor.EXPORT_ROUTE_PLAYTEST_WORKBOOK_JSON_NAME)
        ...
        self.assertEqual(manifest["files"]["appRuntimeAchievements"], "app/runtime_achievements.js")
        ...
        self.assertEqual(manifest["files"]["appRuntimeSfxTransport"], "app/runtime_sfx_transport.js")
        ...
        self.assertEqual(manifest["files"]["appRuntimeRichText"], "app/runtime_rich_text.js")
        self.assertEqual(manifest["files"]["appRuntimeStoryText"], "app/runtime_story_text.js")
        self.assertEqual(manifest["files"]["appRuntimeTextHistory"], "app/runtime_text_history.js")
        self.assertEqual(manifest["files"]["appRuntimeSaveSlots"], "app/runtime_save_slots.js")
    ...
    def test_editor_desktop_build_smoke(self) -> None:
        ...
        self.assertTrue((bundle_dir / "editor_asset_usage.py").is_file())
        ...
        self.assertTrue((bundle_dir / "export_player_template" / "runtime_rich_text.js").is_file())
        self.assertTrue((bundle_dir / "export_player_template" / "runtime_story_text.js").is_file())
        self.assertTrue((bundle_dir / "export_player_template" / "runtime_text_history.js").is_file())
        self.assertTrue((bundle_dir / "export_player_template" / "runtime_save_slots.js").is_file())
        ...
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_MUSIC_TRANSPORT_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SFX_TRANSPORT_NAME).is_file())
        ...
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SYSTEM_MENU_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SAVE_VAULT_NAME).is_file())
        ...
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_TEXT_HISTORY_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_TEXT_HISTORY_OVERLAY_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SURFACE_CACHE_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_DIALOG_PANEL_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SAVE_SLOTS_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SAVE_OVERLAY_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_SAVE_THUMBNAILS_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_RICH_TEXT_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_STORY_TEXT_NAME).is_file())
        self.assertTrue((bundle_dir / "native_runtime" / run_editor.NATIVE_RUNTIME_RICH_TEXT_RENDERER_NAME).is_file())
