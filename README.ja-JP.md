...
- Web / ネイティブ Runtime 共通の耐障害セーブ。通常 / クイック / 自動再開データに対し、ネイティブ側は原子的な置換と直前の有効バックアップ、Web 側は容量逼迫時に再生成可能なサムネイルを省いた回復コピーを使い、破損時は自動復旧を通知します。シーンのサムネイル、システムメニュー、履歴、自動再生、既読スキップ、ボイス回想、タイトル・選択肢・セーブ・設定・図鑑を操作できるゲームパッドナビゲーションにも対応
