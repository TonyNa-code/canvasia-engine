...
- 大きなシーン向けのストーリーカード選択編集。カードを個別選択または Shift で範囲選択し、まとめて移動、複製、削除できます。各操作は 1 回のプロジェクト履歴として Undo でき、構造変更で無効になった BGM 終了範囲は安全に修復されます
- 章をまたいで呼称や用語を安全に変更できるプロジェクト全体のテキストリファクタリング。台詞、ナレーション、選択肢、入力案内、シーン名、章名を対象に選べ、既存翻訳も任意で更新できます。変更前後のプレビュー、古いプレビューの拒否、書き込み失敗時のロールバック、プロジェクト履歴からの一括 Undo に対応
- 任意の時間制限つき選択肢。5 / 10 / 15 / 30 秒または 1–300 秒のカスタム時間、タイムアウト先、ロック時の安全なフォールバックを設定でき、メニューやバックグラウンド中は一時停止します。残り時間はエディタ試遊、Web / native Runtime、セーブ、Ren'Py export で引き継がれます
- 選択した台詞だけに強調、ささやき、安全なカスタム色、ルビ / ふりがなを付けられるノーコード文字演出。文中ポーズや局所速度と併用でき、エディタ試遊、Web Runtime、ネイティブ Runtime、セーブ / 履歴 / 回想のクリーン本文、Ren'Py draft export で同じ意味を保ちます
- 名前、合言葉、調査回答、数値を受け取るプレイヤー入力カード。台詞、ナレーション、選択肢、ロック理由で `{{variable_id}}` を表示でき、Web / native Runtime、セーブ、Ren'Py 下書きで同じルールを使用
- 共通イベント、電話、チュートリアル、反復演出を再利用できるサブシーン呼び出し。ネストした `call`、シーン末尾の自動復帰、明示的な早期 `return` に対応し、呼び出しスタックはエディタ試遊、Web / native Runtime のセーブにも保存され、Ren'Py へ同じ構造で書き出されます
...
- 小道具、前景、オーバーレイ、Cut-in、手紙、エフェクト画像を立ち絵とは別の名前付きレイヤーとして表示 / 更新 / 非表示にできる stage-image カード。前後面、位置、XY オフセット、幅、透明度、回転、反転、重なり順、時間、イージングをエディタ、Web Runtime、ネイティブ Runtime、Ren'Py draft export で共有
...
- 台詞とナレーションごとに、通常の ADV ダイアログ、前文を蓄積して明示的に改ページできる NVL、映画字幕を選択可能。エディタプレビュー、Web Runtime、ネイティブ Runtime、Ren'Py draft export で同じ設定を維持
- 制作者が調整できる話者フォーカス。発話中のキャラクターを自然に前へ出し、ほかの表示中の立ち絵を穏やかに暗くします。エディタ、Web Runtime、ネイティブ Runtime で Off / Soft / Cinematic、強度、切替時間を共有し、静止読書モードでは拡大アニメーションを無効化します
- 制作者が調整できる自動ダイアログカメラ。Soft / Cinematic では発話中のキャラクターへ穏やかにパン・ズームし、ナレーションでは中央構図へ戻ります。エディタ、Web Runtime、ネイティブ Runtime で強度と切替時間を共有し、視覚快適性設定で動きを抑制できます。手動の zoom / pan カードは軸ごとに自動結果より優先されます
- 制作者が調整できる音声連動キャラクター演技。実際の音声エネルギーに合わせて発話中の立ち絵を控えめに上下・拡縮し、Off / Soft / Cinematic、強度、感度をエディタ、Web Runtime、ネイティブ Runtime で共有します。静止読書モードでは動きを停止し、将来の Live2D / 3D アダプターへ `mouthOpen` 信号を渡します
- Runtime 間で共通の精密 BGM 再生。全曲ループ、イントロ後ループ、一度だけ再生、同曲キューでも再スタートの初心者向けプリセットに加え、開始位置、ループ開始 / 終了位置、同曲継続、フェード、音量、ストーリー上の再生範囲を設定でき、エディタ試遊、Web / native Runtime、セーブ、Ren'Py draft で同じ意図を維持します
...
- Web / ネイティブ Runtime 共通の耐障害セーブ。重要な分岐セーブをワンクリックで保護でき、読み込みは維持したまま誤上書きをボタン、ショートカット、保存処理の各層で防止します。通常 / クイック / 自動再開データに対し、ネイティブ側は原子的な置換と直前の有効バックアップ、Web 側は容量逼迫時に再生成可能なサムネイルを省いた回復コピーを使い、破損時は自動復旧を通知します。シーンのサムネイル、システムメニュー、履歴、自動再生、既読スキップ、ボイス回想、タイトル・選択肢・セーブ・設定・図鑑を操作できるゲームパッドナビゲーションにも対応
- Web、3 デスクトップ版、モバイル Web、ネイティブ Runtime 共通の検索可能なテキスト履歴。台詞 / シーンの全文検索、話者フィルター、ボイス付き項目のみの表示、件数確認、安全なボイス再生に対応し、フィルター操作はストーリー位置や旧セーブ形式を変更しません
...
- ストーリーワークスペース内のシーン演出リハーサルボード。すべてのカードを台詞、舞台、音声、演出、ルートの 5 レーンへ揃え、コンパクト表示と完全な絵コンテ表示を切り替えられます。長いシーンは選択中のビートを中心に表示し、キーボードでの移動、元カードへの復帰、任意のビートからの試遊に対応します
...
- モバイル Web 向けの没入型タッチ読書モード。タッチ / 狭幅画面を自動検出し、セーフエリア対応の全画面ステージ、親指で届く Backlog / Auto / Hide / Menu レール、上スワイプの履歴表示と下スワイプの台詞枠切替を提供します。選択肢や入力欄はジェスチャー対象から除外され、作者既定値とプレイヤーの永続設定にも対応します
...
| 領域 | 状態 | 説明 |
| --- | --- | --- |
| ストーリー / 分岐編集 | Available | カード、選択肢、ジャンプ、再利用サブシーンの呼び出し / 復帰、セーブ単位 / 周回記憶の変数、条件分岐、入口到達可能性チェック、リズム分析に連動した配方提案つきシーン制作ボード、フル台本、Ren'Py draft export、ボイス制作シート、シーングラフ確認、台本テキストからカードへのインポートに対応。 |
...
| Web / Desktop 書き出し | Preview | Web 試遊パッケージとデスクトップパッケージは、低負荷の適応型ゲームパッド監視、方向フォーカス移動、キーボード / マウス操作、モバイルブラウザ向けセーフエリア対応タッチ読書に対応。署名と公証は Release notes に従います。 |
| ネイティブ Runtime | Preview | コア再生、設定、ローカル保存のシーンサムネイル付きビジュアルセーブ、ゲームパッドナビゲーションと読書ショートカット、履歴、自動再生、変数・舞台・音楽・画面効果を復元する段階的ロールバック、動画フォールバック、3D レポート、初期図鑑システム、Web / native Runtime coverage checks をカバー。画像のない旧セーブもそのまま読み込めます。背景、立ち絵、stage image、9-slice UI、静的フィルター、合成済み会話パネルは性能プロファイル別の上限付き描画キャッシュを再利用し、毎フレームの再スケールや再合成を避けます。診断画面では命中率、合成時間、最大コスト、推定メモリ、破棄数、上限超過を確認できます。 |
| Mobile Web Runtime | Preview | モバイルブラウザでは全画面ステージ、親指操作レール、履歴シート、上下スワイプ、セーフエリア、タッチモード記憶を利用できます。iOS / Android のネイティブ配布と音声ポリシー認証は引き続き実験計画段階です。 |
...
気軽に共有するなら Web 試遊パッケージが最も簡単です。Web / desktop / native Runtime には `runtime_preload_manifest.json` と `RUNTIME_PRELOAD_REPORT.md` も含まれ、最初のシーンや序盤ルートの背景、立ち絵、音声を優先的に準備します。Web Runtime は省データ / 低速回線で同時ロード数を抑え、実測時間・スループット・遅い素材を記録し、ルート prefetch の重複起動と失敗素材の誤キャッシュを防ぎます。エディタの inspection page では、書き出し前に Runtime preload budget report を出力し、入口シーンの重い素材、欠落ファイル、シーン単位のロード hotspots を確認できます。ネイティブ Runtime は同じ manifest を読み込み、critical な画像と短い音声を優先キャッシュした後、実測コストに応じて 1 フレーム当たりの background preload 数を調整します。よりアプリに近いデスクトップ再生フローを検証する場合は、ネイティブ Runtime パッケージを使います。
