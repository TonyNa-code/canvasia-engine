(function attachRuntimeCapabilityMatrixTools(global) {
  ...
  const FALLBACK_CAPABILITY_ROWS = Object.freeze([
    ...
    ["choice", "分支", "full", "full", "支持玩家选项、变量效果、目标场景跳转、按变量条件隐藏或锁定，以及可暂停并随存档恢复的限时自动选择。"],
  ...
  function buildRuntimeAcceptanceChecklist(rows = [], summary = {}) {
    ...
    if (hasUsedType(usedTypeMap, "video_play")) {
      addItem({
        ...
        target: "native",
        severity: "warn",
        title: "视频播放要验裁段、循环、读档续播和音画同步",
        detail: "带 OP / ED / 插入视频的项目，要检查自动或手动起播、裁段终点、循环退出、播放中存读档、失败兜底，以及回到下一张剧情卡片是否稳定。",
