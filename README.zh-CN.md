...
- 正式存档 / 读档、快速存读档与原生 Runtime 场景缩略图、系统菜单
...
| 原生 Runtime | Preview | 已覆盖核心播放链、带场景缩略图的可视化存档、设置、历史文本、自动播放、可恢复变量 / 舞台 / 音乐 / 视觉效果的逐步剧情回退、视频兜底、3D 资产清单与第一批资料馆；缩略图只写入玩家本机的引擎数据目录，旧存档没有图片时仍可正常读取；编辑器巡检会生成 Web / 原生 Runtime 覆盖矩阵、导出验收清单和 VN 基础能力成熟度摘要。 |
