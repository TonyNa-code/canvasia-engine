...
- 可视化剧情编辑器
- 剧情卡片剪辑选区：可逐张勾选或按住 Shift 连续选择，再把整组选区一起移动、复制或删除；每次批量修改只写入一条可撤销的项目历史，结构调整后失效的 BGM 自定义结束点会安全回退
- 跨章节剧情重构台：可按角色台词、旁白、选项、输入提示、场景名和章节名选择范围，批量修改称呼或术语；支持同时更新已有译文，执行前逐条查看改前 / 改后，预览后项目若有变化会要求重新检查，写入失败自动回滚，并可通过项目历史一次撤销
- 项目中心与空白新建项目
- 新手模式 / 高级模式分层
- 上下文感知指挥面板：可用 Cmd/Ctrl+K 快速搜索项目开工、页面跳转、推荐下一步、最近常用命令、剧情插卡、场景模板、主题切换、新手教程与导出入口
- 商业段落配方：可一键插入 OP 前导、日常对话节奏、好感度选项、悬念线索、关系揭露、分支汇合、高潮演出段、ED / 片尾、BGM 范围调度和带变量后果的分支骨架；每个选项还可设置始终可选、条件隐藏或条件锁定，并按好感度、道具、路线旗标等变量显示解锁提示
- 可选限时选项：可直接选择 5 / 10 / 15 / 30 秒或自定义 1–300 秒，并指定超时后自动进入的分支；若目标届时被隐藏或锁定，会安全回退到第一个可选分支。打开菜单、存读档、教程或切到后台时倒计时暂停，剩余时间会随编辑器试玩、网页 / 原生 Runtime 存档恢复，并可导出到 Ren'Py
- 编辑器试玩、网页 Runtime 与原生 Runtime 共用后台安全播放：切换应用或最小化窗口时会暂停剧情自动前进、限时选项、阻塞视频、打字机推进和片尾，回来后按剩余时间继续，不会因为挂在后台而跳过剧情；原生端还会进入低频待机，减少后台 CPU 占用
- 句内节奏：编辑台词或旁白时，可直接用按钮在光标处加入短停顿 / 长停顿，或把选中的文字设为慢速、快速、瞬间显示，再恢复默认语速；编辑器试玩、网页 Runtime、原生 Runtime、存档、历史、回想与 Ren'Py 草稿共用同一套安全规则，玩家选择“瞬间显示”时始终优先尊重玩家设置
- 剧情富文本：不用写代码，只需选中台词或旁白中的一小段，就能设置重点强调、轻声低语、安全自定义颜色和汉字注音；可与句内停顿 / 局部语速同时使用，并在编辑器试玩、网页 Runtime、原生 Runtime、干净的存档 / 历史 / 回想正文及 Ren'Py 草稿中保持一致，不接受不安全的原始 HTML
- 玩家输入与正文变量：可用“询问玩家”卡片收集姓名、口令、调查答案或数字，并在台词、旁白、选项和锁定提示中用 `{{变量ID}}` 显示；网页试玩、原生 Runtime、存档和 Ren'Py 草稿导出共用同一套规则
- 可复用子场景：可调用公共事件、电话、教程或重复演出，播放完自动回到调用处，也可用“提前返回”卡片结束；支持嵌套调用，调用栈会随编辑器试玩、网页 / 原生 Runtime 存档恢复，并导出为 Ren'Py `call` / `return`
- 场景演出配方：可一键把镜头、滤镜、景深、粒子、停顿、闪屏、震动、BGM 和黑场淡入淡出插成可继续编辑的卡片，用于告白特写、悬疑压迫、雨夜回忆、爆点冲击和收束留白等常见 VN 氛围
- 独立舞台贴图卡：可把信件、手机、道具、前景、遮罩、Cut-in 和特效纹理作为命名图层显示、移动或隐藏，并自由设置前后层、基础位置、XY 偏移、宽度、透明度、旋转、镜像、叠放顺序、动作时长和缓动；编辑器预览、网页 Runtime、原生 Runtime 与 Ren'Py 草稿导出共用同一套数据规则
- 手写剧本转剧情卡片：可把文档 / 备忘录里的 `角色：台词`、`角色 "台词"`、旁白、连续选项、`- 选项 -> 场景 [好感 +1; 已见面=true]` 变量后果、`设置 好感 为 2` / `add affection +1` 独立变量卡、`if 好感 >= 2 -> 好结局 else -> 普通结局` 条件分支，以及 `scene / show / move / hide / show 或 move ... scale / x / y / opacity / layer / flip / duration / easing / stage image show / update / hide / speed slow / normal / fast / instant / play music / stop music / play sound / play video / wait / pause / 等待 / 停顿 / shake / flash / zoom / pan / filter / blur / particle / credits / voice / jump / call / return` 等轻量演出、文字速度、音频、视频、节奏停顿、镜头、角色舞台、氛围和路线指令预览后插入当前场景；视频指令还可追加 `manual / auto`、`loop / once`、`resume / restart`、起止秒数、适配和音量规则
- 角色、素材、台词台本、配音工作流
...
- 跨 Runtime 音效与环境声导演：提供短促效果、环境循环、界面提示、持续心跳等小白预设；支持短音效叠加、效果 / 环境 / UI 逻辑声道、循环环境声、同素材续播或重播、淡入、替换淡出、按声道停止、存档恢复，以及 Ren'Py 多声道音效池导出
- 跨 Runtime 视频导演：提供 OP / ED、剧情过场、循环氛围和手动播放四组小白预设，也可自定义自动 / 手动起播、精确起止秒数、单次 / 安全循环、读档重播 / 续播、画面适配、音量和跳过规则；编辑器会真实内嵌试玩，网页与原生 Runtime 会保存播放位置，Ren'Py 草稿对暂时无法等价迁移的高级规则会给出复核提示而不是静默丢失
...
- 作者可启用自动对话镜头：柔和 / 电影感模式会随当前说话者的位置轻微横移和推近，旁白自动回归中性构图；编辑器试玩、网页 Runtime 与原生 Runtime 共用强度和过渡时间，舒适阅读方案会降低或关闭运动，手写镜头缩放 / 平移卡仍按轴优先覆盖自动结果
- 作者可启用语音驱动角色演技：实际语音能量会带动当前发言立绘产生克制的呼吸与起伏，支持关闭 / 柔和 / 电影感三档以及强度、灵敏度调节；编辑器试玩、网页 Runtime 与原生 Runtime 共用同一配置，静态阅读方案会完全停用运动，并向未来的 Live2D / 3D 适配层提供 `mouthOpen` 嘴型参数
- 首页今日工作台、最终发表门禁、项目巡检、跨模块生产待办队列 / CSV、Runtime 覆盖矩阵 / CSV、Runtime 导出验收清单、VN 基础能力成熟度体检、项目医生小白修复向导、项目医生回执导出 / 复制 / 编号、成品目标路线、一键安全修复低风险结构问题、一键发布前修复顺序（会把首屏加载风险、VN 基础缺口和具体路线试玩阻塞项纳入排序）、一键发布前整理（会先创建安全检查点，再安全处理长文本、台词质量体检、基础演出、BGM 范围 / 淡入淡出、存档位、文本框可读性、UI 皮肤和字体绑定，并生成回执）、视觉小说基础质感体检、带节奏体检和一键配方建议的场景生产看板 / CSV、语音制作清单 / CSV、选项后果表 / CSV、变量影响表 / CSV、发布前路线试玩工作簿 / CSV、会从标准路线计划提取具体分支 / 结局阻塞项的生产待办、素材依赖表 / CSV、素材体积雷达 / CSV、Runtime 首屏加载预算 / CSV、可解锁内容清单 / CSV、BGM 调度表 / CSV、角色舞台调度表 / CSV、演出时间轴 / CSV、多语言覆盖报告 / CSV、翻译 CSV 安全导入、素材性能预算、会把首屏加载问题纳入队列的生产待办、发布总控 Markdown / JSON 报告、测试员试玩工单 / CSV、测试反馈模板 / CSV、反馈导入摘要、发布验收清单、带条件 / 否则分支变量预设的分支感知自动回归试玩路线测试
- 作者自定义成就卡：可在任意剧情点设置稳定 ID、标题、说明、分类、达成条件、解锁前隐藏和图标；网页 / 原生 Runtime 会持久保存并显示解锁通知，巡检会提示重复 ID，Ren'Py 草稿会同步导出 `achievement.register` / `achievement.grant`
...
- 网页 / 原生 Runtime 的防崩溃正式存档、快速存读档与续玩记录：关键分支档可一键设为“重要存档”，保护后仍可读取，但按钮、快捷键与底层写入都会阻止误覆盖；网页与桌面网页 Runtime 可导出项目专属 `.canvasia-save.json`，原生 Runtime 则在系统菜单提供“数据保险箱”，用独立 `.canvasia-save` 文件打包全部档位、续玩记录、解锁进度、跨周目记忆、玩家档案和体验设置。两条路径都会先核对项目身份、格式版本与完整性，再由玩家二次确认；原生恢复还会自动建立恢复前安全点，任一记录写入失败都会整组回滚，不留下半恢复状态。原生端同时保留原子替换、上一份有效备份、损坏自动恢复提示、场景缩略图，以及覆盖标题页、选项、存档、设置、历史和资料馆的手柄操作
- 网页、三平台桌面包、移动网页与原生 Runtime 共用的可检索文本历史：可按台词 / 场景全文搜索、按角色筛选、只看带语音记录、查看命中数量并安全重听语音；筛选只改变阅读视图，不改变剧情位置，也不要求升级旧存档
- 原生 Runtime 电影式片尾：按作者设置的时长真实滚动，支持深色 / 浅色 / 透明背景、可跳过规则与播放结束自动续接；玩家使用静态阅读方案时会自动改为无滚动分页显示
- 作者可为变量选择“跟随存档”或“跨周目记忆”：跨周目变量会在新游戏、读取旧存档和剧情回退后继续保留最新值，网页试玩、原生 Runtime 与 Ren'Py 导出使用同一语义；玩家也可在系统菜单单独重置跨周目记忆，不会删除正式存档
- 入口可达性路线分析：可检查坏链、孤立场景、入口实际走不到的场景、分支深度、结局候选、可打到结局路径预览，并可导出路线试玩手册 / CSV；巡检页会生成发布前路线试玩工作簿，把断点修复、分支逐条覆盖、结局完整跑通和自动回归优先种子拆成可执行步骤
- 编辑器试玩飞行记录器：只跟随当前有效时间线，自动记录变量前后值、选项 / 条件结果、背景 / BGM / 立绘 / 舞台贴图 / 特效调度；可直接跳回记录步骤、打开源卡片，并导出易读 Markdown 或完整 JSON 诊断记录
- 剧情工作区导演排练台：把每张卡片对齐到对白、舞台、声音、演出和路线五条轨道，可在节拍速览与完整分镜间切换；长场景会围绕当前卡片分窗显示，支持键盘逐拍浏览、回到源卡片，并从任意节拍直接试玩
- 场景节奏体检：可检查可试玩节奏、长文本、空台词 / 空旁白、附近重复文本、过长选项、演出过平、假选项、缺少收尾提示，并在场景生产看板里给出下一步建议
- Ren'Py 草稿导出：可把场景、对白、旁白、ADV / NVL / 电影字幕呈现、逐句文字速度、带变量条件的选项显示、变量、条件分支、语音、带裁段 / 音量 / 时长的 video 过场、背景转场时长、立绘 / BGM 音量与循环提示、指定范围 BGM 停止 / 淡出、音效音量、自定义立绘位置 / 缩放 / 透明度 / 镜像 / 层级、命名道具 / 前景 / Cut-in 舞台贴图层、带时长的滑入 / 升起 / 退场立绘转场、等待、闪屏 / 淡入淡出颜色与时长、基础镜头推近 / 平移 / 滤镜 / 景深模糊、基于 SnowBlossom 的氛围粒子、片尾字幕和跳转转换成 `.rpy` 草稿，并额外导出迁移备注标出需要人工复核的复杂时序和自定义演出
- 项目级成品 UI 皮肤、UI Kit 部件绑定、九宫格贴图、按钮多状态贴图、布局位置微调、网页 / 桌面试玩共用的字体素材绑定与安全回退，以及视觉小说文本框设计；可导出便携的 `.canvasia-ui-kit.json`，把整套皮肤、文本框配置、引用的 UI 图片与字体、署名授权信息和完整性校验一起带到另一个项目，导入后会自动换成新项目自己的素材 ID
- 网页与原生 Runtime 共用的玩家自定义按键：安全保留关闭 / 帮助 / 存档键，冲突时自动交换，可一键恢复推荐键位，操作帮助会同步显示当前键位
- 移动网页成品的沉浸式触控阅读：自动识别触屏 / 窄屏并切换为安全区适配的全舞台布局，底部提供拇指可达的回看 / 自动 / 隐框 / 菜单控制轨；画面上滑打开可跳转回看、下滑隐藏或恢复文本框，按钮、选项和输入框不会被手势误触；作者可设置默认策略，玩家覆盖后会保存在本机
- 编辑器试玩、网页 Runtime 与原生 Runtime 共用的玩家视觉舒适度：可保留作品原始演出，也可切到柔和或静态模式；柔和模式会降低震动、闪屏和转场强度，静态模式会停止短暂闪烁、粒子、角色呼吸与转场动画，但不会改写创作者的项目数据
- 编辑器试玩、网页 Runtime 与原生 Runtime 共用的一键阅读方案：原作演出、舒适阅读、大字阅读、静态阅读会联动文字速度、字号、文本框可见度和视觉舒适度；玩家继续逐项微调时会自动标记为“自定义组合”，设置只保存在玩家本地
- EXTRA 回想馆、图鉴馆、成就馆、章节回放、结局回放、语音回听，并可导出可解锁内容清单 / CSV 检查 CG、BGM、语音回听、角色图鉴、地点图鉴、章节、结局和成就是否齐全
- 高级粒子系统、项目级粒子预设库，以及按项目性能档位分配的组合层总预算；成品遇到持续帧率压力时会自动降低或恢复粒子密度，不会打断剧情、语音和音乐
...
| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| 剧情 / 分支编辑 | 可用 | 支持可视化卡片、玩家命名 / 问答输入、正文变量显示、跟随存档 / 跨周目变量、选项跳转、可复用子场景调用 / 返回、条件隐藏 / 锁定与解锁提示、变量后果、入口可达性路线检查、发布前路线试玩工作簿、带节奏体检配方建议的场景生产看板、台词质量体检、完整剧本台本、Ren'Py 草稿导出、语音制作清单、变量影响表、场景树筛选、手写剧本文本识别插入，并可通过命令面板一键套用“第一段可试玩”剧情骨架。 |
...
| 项目安全网 | 可用 | 支持自动快照、版本恢复、崩溃恢复、正式存档 / 读档、网页 / 桌面玩家存档备份迁移、首页今日工作台、最终发表门禁、跨模块生产待办队列、项目医生修复队列、项目医生可追溯回执、成品目标路线、入口场景 / 章节顺序 / 场景顺序的一键安全修复、一键发布前整理回执、发布前检查、视觉小说基础质感体检、VN 基础能力成熟度体检、可解锁内容就绪度与发布总控 Markdown / JSON 报告导出。 |
| 成品 UI 自定义 | 可用 | 支持项目级 UI 皮肤、按钮多状态、九宫格贴图、布局位置、字体素材绑定与安全回退、视觉小说文本框设计，以及包含依赖素材、授权信息、完整性校验和项目内 ID 自动重绑的 UI Kit 整包导入 / 导出；导入写入失败会整组回滚，不留下半套皮肤。 |
| EXTRA / 回想系统 | 可用 | 支持图鉴馆、回想馆、成就馆、章节回放、结局回放与语音回听；可解锁内容清单会检查缺失图库文件、语音回听缺口、角色图鉴视觉素材和不可达结局。 |
| 粒子与演出 | 可用 | 支持高级粒子预设、项目级自定义粒子、组合层总量预算、网页 / 原生 Runtime 自适应粒子画质、场景演出配方、镜头、滤镜、闪屏、震动，以及可独立显示 / 移动 / 隐藏的道具、前景和 Cut-in 舞台贴图层。 |
| Live2D / 3D 资产 | Preview | 支持 Live2D、3D 角色模型和 3D 场景素材导入；原生 Runtime 会生成 3D 资产结构、材质贴图槽、动画通道和依赖清单，并可把风险定位回编辑器素材库。 |
| 网页 / 桌面导出 | Preview | 网页试玩包与三平台桌面包支持低开销自适应手柄轮询、方向焦点导航、可持久化自定义键位、鼠标操作，以及移动浏览器安全区适配的沉浸式触控阅读；签名、公证和安装器状态以 Release notes 为准。 |
| 原生 Runtime | Preview | 已覆盖核心播放链、电影式滚动 / 静态分页片尾、可持久化自定义键位、角色 / 旁白独立语音混音、带场景缩略图的可视化存档、手柄导航与阅读快捷动作、设置、历史文本、自动播放、可恢复变量 / 舞台 / 音乐 / 视觉效果的逐步剧情回退、后台安全时间线暂停与低频待机、自动 / 手动与单次 / 循环视频调度、视频播放位置存读档及 PyAV / OpenCV / 系统播放器兜底、3D 资产清单与第一批资料馆；缩略图只写入玩家本机的引擎数据目录，旧存档没有图片时仍可正常读取；背景、立绘、舞台贴图、九宫格 UI、静态滤镜和完整对话框底板使用按性能档位限额的绘制缓存，不再每帧重复缩放或合成；性能诊断会显示命中率、合成耗时、峰值成本、估算内存、淘汰和超预算绕过；编辑器巡检会生成 Web / 原生 Runtime 覆盖矩阵、导出验收清单和 VN 基础能力成熟度摘要。 |
| 移动网页 Runtime | Preview | 手机浏览器已支持沉浸式全舞台、拇指操作轨、剧情回看面板、上下滑阅读手势、安全区与玩家触控模式记忆；原生 iOS / Android 安装包和移动音频策略认证仍处于实验规划。 |
...
- [`export_choice_consequence_sheet.py`](export_choice_consequence_sheet.py)
  导出包侧选项后果、变量效果、坏跳转、重复选项和假按钮复查报告生成器

- [`export_variable_influence_sheet.py`](export_variable_influence_sheet.py)
  导出包侧变量定义、读写位置、条件读取、坏引用和类型错误复查报告生成器
...
- [`export_route_playtest_workbook.py`](export_route_playtest_workbook.py)
  导出包侧路线试玩工作簿生成器，把分支、结局、坏链和入口不可达项拆成可执行 QA 步骤
...
- [`prototype_editor/modules`](prototype_editor/modules)
  前端纯逻辑模块，覆盖素材目录、创作助手、路线分析、试玩飞行记录、项目医生、成品目标路线、发布总控、变量、外观主题、项目历史等可单独测试的能力
...
- `网页试玩包`：适合快速预览、网页分发和轻量测试；导出包会附带 `runtime_preload_manifest.json` 与 `RUNTIME_PRELOAD_REPORT.md`。Runtime 会优先预热首屏和早期路线素材，并根据省流量 / 较慢网络限制并发，记录单项耗时、吞吐和慢素材；路线预取由独立资产管线去重管理，失败项不会误记为缓存，并可在冷却后有界重试。编辑器“项目巡检”页也可在导出前生成 Runtime 首屏加载预算报告，提前发现入口场景大素材、缺文件和场景热点。
...
- `原生 Runtime 包`：Preview 路线，已覆盖标题页主菜单、基础剧情主链、正式存档/读档、系统菜单设置项、语言切换、文本历史、自动播放、已读快进、逐步剧情回退、项目字体、资源预热清单读取、玩家档案/自动续玩、基础粒子与镜头演出、后台安全时间线暂停、3D 资产结构 / 依赖清单、可选 PyAV/FFmpeg 音画同步内嵌视频播放、OpenCV 画面兜底、系统播放器桥接兜底、第一批资料馆，以及随包生成的发布候选总报告、发布总控报告与三系统验收清单；窗口失焦或最小化时会停止剧情计时与画面推进并降低后台帧率，恢复后从剩余进度继续；启动时会优先缓存关键图片和短音频，后续素材按实测单项耗时自适应调整每帧预热量，诊断页会显示平均耗时、峰值与慢项数量，并确认 BGM / 视频这类流媒体路径是否就绪。
