...
- 网页 / 原生 Runtime 的防崩溃正式存档、快速存读档与续玩记录：关键分支档可一键设为“重要存档”，保护后仍可读取，但按钮、快捷键与底层写入都会阻止误覆盖；原生端使用原子替换和上一份有效备份，网页端在空间压力下会省略可重建缩略图来保住进度，检测到损坏时自动恢复并明确提示；另有原生场景缩略图、系统菜单，以及覆盖标题页、选项、存档、设置、历史和资料馆的手柄操作，方向长按可连续移动，确认 / 返回 / 菜单保持单次触发
