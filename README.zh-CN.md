...
- 首页今日工作台、最终发表门禁、项目巡检、项目医生小白修复向导、项目医生回执导出 / 复制 / 编号、成品目标路线、一键安全修复低风险结构问题、一键发布前修复顺序、视觉小说基础质感体检、BGM 调度表 / CSV、角色舞台调度表 / CSV、多语言覆盖报告 / CSV、素材性能预算、发布总控 Markdown / JSON 报告、测试员试玩工单 / CSV、测试反馈模板 / CSV、反馈导入摘要、发布验收清单、自动回归试玩路线测试
...
| 多语言 / 国际化 | Preview | 支持项目默认语言与可切换语言配置；可在项目巡检中导出缺翻译 / 疑似占位翻译覆盖报告；导出数据会保留场景、章节、台词、选项和角色名翻译，网页 Runtime 与原生 Runtime 可切换语言并在缺失翻译时自动回退。 |
