...
所有可试玩 / 可运行导出包都会随包生成 `unlockable_content_manifest.json` 和 `unlockable_content_report.md`。JSON 适合自动化验收读取，Markdown 报告适合测试员直接查看；两者都会记录 CG 图鉴、音乐回想、语音回听、角色 / 地点 / 旁白 / 关系图鉴、章节回放、结局收集和成就覆盖情况，方便确认 EXTRA / 回想内容有没有真的进入成品包。
