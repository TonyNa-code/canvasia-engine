...
- 网页 / 原生 Runtime 的防崩溃正式存档、快速存读档与续玩记录：关键分支档可一键设为“重要存档”，保护后仍可读取，但按钮、快捷键与底层写入都会阻止误覆盖；网页与三平台桌面 Runtime 可在系统菜单把设置、进度、解锁记录和全部档位导出为项目专属 `.canvasia-save.json`，换浏览器或换电脑后先只核对项目身份、格式版本与完整性，再由玩家二次确认恢复；文件过大时只省略可重建缩略图，任一写入失败会全量回滚，不会留下半恢复状态。原生端使用原子替换和上一份有效备份，检测到损坏时自动恢复并明确提示；另有原生场景缩略图、系统菜单，以及覆盖标题页、选项、存档、设置、历史和资料馆的手柄操作，方向长按可连续移动，确认 / 返回 / 菜单保持单次触发
