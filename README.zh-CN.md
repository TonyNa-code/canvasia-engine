...
- 网页 / 原生 Runtime 的防崩溃正式存档、快速存读档与续玩记录：原生端使用原子替换和上一份有效备份，网页端在空间压力下会省略可重建缩略图来保住进度，检测到损坏时自动恢复并明确提示；另有原生场景缩略图、系统菜单，以及覆盖标题页、选项、存档、设置、历史和资料馆的手柄操作，方向长按可连续移动，确认 / 返回 / 菜单保持单次触发
...
| 原生 Runtime | Preview | 已覆盖核心播放链、电影式滚动 / 静态分页片尾、可持久化自定义键位、角色 / 旁白独立语音混音、带场景缩略图的可视化存档、手柄导航与阅读快捷动作、设置、历史文本、自动播放、可恢复变量 / 舞台 / 音乐 / 视觉效果的逐步剧情回退、自动 / 手动与单次 / 循环视频调度、视频播放位置存读档及 PyAV / OpenCV / 系统播放器兜底、3D 资产清单与第一批资料馆；缩略图只写入玩家本机的引擎数据目录，旧存档没有图片时仍可正常读取；编辑器巡检会生成 Web / 原生 Runtime 覆盖矩阵、导出验收清单和 VN 基础能力成熟度摘要。 |
