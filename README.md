...
- Ren'Py draft export that converts scenes, dialogue, narration, per-line text speed tags, choices, variables, condition branches, voice cues, video cutscenes, basic background / sprite / BGM cues, custom character position / scale / opacity / flip / layer transforms, waits, simple screen effects, credits, and jumps into a reviewable `.rpy` starter file with a migration review manifest for timing-sensitive or custom effects
