...
- Background-safe playback across editor preview, Web Runtime, and native Runtime: switching apps or minimizing the window pauses story auto-advance, timed choices, blocking video, typewriter progression, and credits, then resumes from the remaining time instead of skipping content; the native player also enters a low-frequency idle loop to reduce background CPU use
...
- Crash-resistant formal save/load and quick save/load across Web and native Runtime, including one-click protection for important branch saves so they remain readable but cannot be accidentally overwritten. Web and desktop-Web players can export project-locked `.canvasia-save.json` backups; the native Runtime provides a player-facing Data Vault with independent `.canvasia-save` archives covering saves, auto-resume, unlocks, cross-playthrough memory, profiles, and settings. Both paths verify format, project identity, and integrity before restore, require explicit confirmation, create a pre-restore safety point, and roll back the full transaction if any record write fails. Native saves also retain atomic writes, a rolling valid backup, automatic recovery notices, and scene thumbnails; both Runtime paths provide system menus, text history, autoplay, skip-read, voice replay, and controller-first navigation with directional hold-repeat across title screens, choices, saves, settings, history, and archives
...
- Custom game UI skins, UI Kit binding, nine-slice textures, button states, layout controls, bound font assets with safe system-font fallback across Web / desktop playback, and visual novel textbox design; a portable `.canvasia-ui-kit.json` package can carry the complete skin, textbox settings, referenced UI artwork, font files, attribution metadata, and integrity checksum into another project without broken asset IDs
...
- Web playable, desktop, and native Runtime exports with size-aware preload manifests, connection-aware and measured asset warming, bounded route-aware next-scene / branch prefetching, cache-safe failure retry, adaptive native per-frame budgets, and editor-side startup-pressure reports for smoother first-scene and scene-transition loading, plus editor desktop builds
...
| Game UI Customization | Available | Project UI skins, button states, nine-slice images, layout tuning, bound font assets with safe fallback, textbox styling, and transactional UI Kit import / export with embedded dependencies, rights metadata, integrity checks, and automatic project-local asset rebinding. |
...
| Native Runtime | Preview | Covers the core playback path, cinematic scrolling / static-paged credits, settings, persistent custom keyboard mappings, persistent per-character / narrator voice mixing, visual save slots with locally stored scene thumbnails, controller navigation and reading shortcuts, history, autoplay, state-safe story rollback, background-safe timeline suspension with low-frequency idle rendering, automatic / manual and single / looping video transport with saved-position recovery plus PyAV / OpenCV / system-player fallbacks, 3D reports, first archive systems, and editor-side Runtime capability matrices with Web / native acceptance checklists plus VN essentials maturity summaries. A bounded, performance-profile-aware surface cache reuses scaled backgrounds, sprites, stage images, nine-slice UI, static filter layers, and fully composed dialogue-panel chrome instead of rebuilding them every frame; preload diagnostics now sample per-asset latency, retain a bounded slow-asset list, and reduce or restore background work per frame from measured cost. Diagnostics also expose cache hit rate, build time, peak build cost, memory estimate, evictions, and over-budget bypasses. |
