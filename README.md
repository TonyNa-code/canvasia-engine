...
- Asset management for backgrounds, character sprites, CGs, BGM, SFX, voice, fonts, UI assets, Live2D files, 3D models, and 3D scenes, with dependency reports that show where each asset is used and rights / credits sheets for license, source, commercial-use, placeholder, and AI provenance checks
...
| Asset Management | Available | Import, replace, delete, usage protection, dependency reports, rights / credits reports for license, source, commercial-use, placeholder, and AI provenance checks, file-size budget hints, and optional OpenAI Image generation with style presets, sprite expression binding, prompt, model, format, and returned-file validation. |
