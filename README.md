...
- Crash-resistant formal save/load and quick save/load across Web and native Runtime, including one-click protection for important branch saves so they remain readable but cannot be accidentally overwritten; Web and desktop players can export settings, progress, unlocks, and every save slot as a project-locked `.canvasia-save.json` backup, validate it without touching current data, then explicitly restore it on another browser or computer. Backups carry format, project-identity, and integrity checks, strip only reproducible thumbnails when size pressure requires it, and roll back the entire import if any browser write fails. Native saves retain atomic writes, a rolling valid backup, automatic recovery notices, and scene thumbnails; both Runtime paths also provide system menus, text history, autoplay, skip-read, voice replay, and controller-first navigation with directional hold-repeat across title screens, choices, saves, settings, history, and archives
