...
- Plain-text and Ren'Py-style script import that previews `Character: line`, quoted dialogue, narration, choices with `[affection +1; flag=true]` consequences, `scene`, `show`, `hide`, `show ... scale / x / y / opacity / layer / flip`, per-line `speed slow / normal / fast / instant`, `if affection >= 2 -> good else -> normal`, `play / stop music`, `play sound`, `play video`, `shake`, `flash`, `zoom`, `pan`, `filter`, `blur`, `particle`, `credits`, `voice`, and `jump` cues as editable story / staging / text-speed / variable / condition / audio / video / camera / atmosphere / route cards
