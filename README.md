...
- Ren'Py draft export that converts scenes, dialogue, narration, per-line text speed tags, choices, variables, condition branches, voice cues, video cutscenes, background transition timing, sprite / BGM cues with volume / loop hints, SFX volume, custom character position / scale / opacity / flip / layer transforms, waits, screen flash / fade color and duration cues, basic camera zoom / pan / filter / blur cues, credits, and jumps into a reviewable `.rpy` starter file with a migration review manifest for timing-sensitive or custom effects
