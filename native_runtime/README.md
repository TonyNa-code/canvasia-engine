...
- 项目级字体族 / 字体素材同步
- Runtime 资源预热清单读取：启动时缓存关键图片 / 短音频，后续素材后台分批预热，并确认 BGM / 视频流媒体路径
- 文本历史、自动播放、已读快进
- 基础粒子特效表现
...
| 资源预热 | 已读取导出包 `runtime_preload_manifest.json`，关键资源启动预热，非关键资源通过 `runtime_preload.py` 小步后台队列继续准备 | 后续补预热耗时统计、低内存模式和更多素材类型 |
